...
export default class IdolInfo extends Component {
  ...
  render() {
    ...
    const computedPercent = hasBuyStock ? `持股占比：${parseFloat(
    ...
    return (
      <AtTabsPane current={this.state.current} index={0}>
        <AtList>
          ...
          <AtListItem title='每股价格' extraText={`￥${idol.stock_price} / 股`} />
          {
            boss
              ?
              <navigator
          ...
          <AtListItem title='总发行额' extraText={hasLimited ? `${idol.max_stock_count} 股` : '无上限'} />
          <AtListItem title='已认购额' extraText={`${idol.star_count} 股`} />
          <AtListItem title='我持有股' extraText={hasBuyStock ? `${idol.has_star} 股` : '未入股'} note={computedPercent} />
